...
def creer_pages(nombre_pages):
    ...
    for i in range(nombre_pages):
        ...
        page = cmds.polyCube(name=f"Page_{i+1:02d}", w=width, h=0.02, d=depth, sx=12, sy=12, sz=12)[0]      ###MODIFIE MODIFIE MODIFE
...
def ralentir_pages(frame_debut_ralentissement, duree_ralentissement=30):
    ...
    for ctrl in all_controls:
        ...
        keyframes_translate = cmds.keyframe(ctrl, attribute='translateY', query=True, time=(frame_debut_ralentissement, 100000))
    ...
    # Récupérer tous les bends
    all_bends = cmds.listRelatives("Bend_GRP", children=True, type='transform')
    if not all_bends:
        return

    # Parcourir chaque bend pour modifier son animation
    # On traite chaque bend individuellement
    # Car certaines pages peuvent être immobiles, d'autres en mouvement
    ...
    for bend in all_bends:
        ...
        keyframes_curvature = cmds.keyframe(bend_node, attribute='curvature', query=True, time=(frame_debut_ralentissement, 100000))
        
        # Si des keyframes existent après cette frame
        if keyframes_curvature and len(keyframes_curvature) > 0:
            # Récupérer la valeur de curvature actuelle à frame_debut_ralentissement
            # time=... évalue l'attribut à cette frame spécifique
            curvature_actuelle = cmds.getAttr(f"{bend}.curvature", time=frame_debut_ralentissement)
            
            # Supprimer toutes les keyframes futures sur "curvature"
            # cutKey supprime les keyframes dans la plage de temps spécifiée
            # +0.01 pour ne pas supprimer la keyframe actuelle
            cmds.cutKey(bend, attribute='curvature', time=(frame_debut_ralentissement + 0.01, 100000))
            
            # Calculer la frame de fin du ralentissement
            frame_fin = frame_debut_ralentissement + duree_ralentissement
            
            # Créer une nouvelle keyframe à frame_fin avec la valeur actuelle
            # La page va donc "geler" progressivement à cette curvature
            cmds.setKeyframe(bend_node, attribute='curvature', value=curvature_actuelle, time=frame_fin)
            
            # Appliquer une tangente linéaire pour un ralentissement progressif
            # On utilise 'linear' plutôt que 'spline' pour un ralentissement uniforme
            # 'spline' créerait une courbe qui pourrait faire bouger un peu la page
            cmds.keyTangent(bend_node, attribute='curvature', time=(frame_debut_ralentissement, frame_fin), outTangentType='linear')
