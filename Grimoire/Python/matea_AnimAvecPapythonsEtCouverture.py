...
def gestion_lights_et_camera(Temps_total, frame_debut, duree_emission, frame_fin_animation):
    ...
    cmds.select("backSky_Grimoire_lgt")
    cmds.move(0, 1000, 0)
    cmds.scale(0.1, 0.1, 0.1)
    cmds.setAttr("backSky_Grimoire_lgtShape.color", 0.011,0.003,0.022)
    ...
    cmds.setKeyframe(renderCam, attribute='scaleX', value=3.5, time = finAnim_plus_emission_2_97)
    cmds.setKeyframe(renderCam, attribute='scaleY', value=3.5, time = finAnim_plus_emission_2_97)
    cmds.setKeyframe(renderCam, attribute='scaleZ', value=3.5, time = finAnim_plus_emission_2_97)
    cmds.setKeyframe(renderCam, attribute='centerOfInterest', value=5, time = finAnim_plus_emission_2_97)

    # Camera se TP chez matea
    debut_Matea = finAnim_plus_emission_2_97 + 1
    cmds.setKeyframe(renderCam, attribute='translateX', value=509.37, time = debut_Matea)
    cmds.setKeyframe(renderCam, attribute='translateY', value=0.602, time = debut_Matea)
    cmds.setKeyframe(renderCam, attribute='translateZ', value=6.212, time = debut_Matea)
    cmds.setKeyframe(renderCam, attribute='rotateX', value=7.849, time = debut_Matea)
    cmds.setKeyframe(renderCam, attribute='rotateY', value=63.004, time = debut_Matea)
    cmds.setKeyframe(renderCam, attribute='rotateZ', value=0.827, time = debut_Matea)
    cmds.setKeyframe(renderCam, attribute='scaleX', value=1, time = debut_Matea)
    cmds.setKeyframe(renderCam, attribute='scaleY', value=1, time = debut_Matea)
    cmds.setKeyframe(renderCam, attribute='scaleZ', value=1, time = debut_Matea)
    cmds.setKeyframe(renderCam, attribute='centerOfInterest', value=5, time = debut_Matea)


    # Travelling Avant 
    fin_Matea = debut_Matea + 100
    cmds.setKeyframe(renderCam, attribute='translateX', value=509.27, time = fin_Matea)
    cmds.setKeyframe(renderCam, attribute='translateY', value=0.619, time = fin_Matea)
    cmds.setKeyframe(renderCam, attribute='translateZ', value=6.158, time = fin_Matea)
    cmds.setKeyframe(renderCam, attribute='rotateX', value=7.849, time = fin_Matea)
    cmds.setKeyframe(renderCam, attribute='rotateY', value=63.004, time = fin_Matea)
    cmds.setKeyframe(renderCam, attribute='rotateZ', value=0.827, time = fin_Matea)
    cmds.setKeyframe(renderCam, attribute='centerOfInterest', value=5, time = fin_Matea) #test sinon 0.059
    ...
    cmds.setKeyframe(backSky, attribute = "visibility", value=0, time=debut_Matea)
    cmds.setKeyframe("sky_lgt", attribute = "visibility", value=1, time=debut_Matea)


    ### DECALER LES KEY FRAMES DES PAPYTHONS 
    # On crée une liste de tous les groupes qu'on vaut parcourir
    ...
    #On récupère tous les descendants de notre liste
    for grp in groupes_racine:
        if cmds.objExists(grp):
            papillons.extend(cmds.listRelatives(grp, allDescendents=True, fullPath=True) or [])

    #Askip ca supprime les doublons, mais j'ai du mal a comprendre pourquoi, merci Reddit
    papillons = list(set(papillons))

    #On peut modifier les keyframes uniquement des curves, et pas des transforms :
    anim_curves = set()

    #pour chaque objet de la liste papillons, on récupère les curves
    for obj in papillons:
        curves = cmds.listConnections(obj, type="animCurve") or []
        anim_curves.update(curves)

    #On ajoute les curves dans anim_curves, pour toutes celles qui ont une animation
    anim_curves = list(anim_curves)

    #on veut connaitre maintenant la 1e frame parmi toutes nos curves
    first_key = min(cmds.keyframe(curve, query=True, timeChange=True)[0] for curve in anim_curves)
    
    #on veut qu'à chaque fois, de manière modulable, les papythons soient a la frame "debut matea" donc on créée un offset
    offset = debut_Matea - first_key

    #et pour chaque curve, on déplace chaque bloc de keyframes pour les faire commencer à "offset" 
